...
enum PendingCommandBuffer {
    ...
    Task {
        task: Box<dyn FnOnce(CommandBufferTaskContext) -> CommandBuffer + Sync + Send>,
    },
}
...
impl PendingCommandBuffers {
    pub fn push_buffers(&mut self, buffers: impl IntoIterator<Item = CommandBuffer>) {
        self.0
            .commands
    ...
    pub fn push_task(
        &mut self,
        task: impl FnOnce(CommandBufferTaskContext) -> CommandBuffer + Sync + Send + 'static,
    ) {
        self.0.commands.push(PendingCommandBuffer::Task {
            task: Box::new(task),
        });
    }
    ...
    pub fn finish(
        &mut self,
        context: CommandBufferTaskContext,
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            finish_parallel(context, commands)
        }
    }
...
fn finish_sequential(
    ...
    commands.into_iter().map(|command| match command {
        ...
        PendingCommandBuffer::Task { encoder, .. } => encoder.finish(),
    })
}
    ...
    let mut finished_encoders = ComputeTaskPool::get().scope(|scope| {
        for (index, command) in commands.into_iter().enumerate() {
            match command {
                ...
                PendingCommandBuffer::Task { task } => {
                    let context = context.clone();
                    scope.spawn(async move { (index, task(context)) });
                }
            }
        }
    });
...
impl RenderContextStateInner {
    fn flush_encoder(&mut self) {
        if let Some(encoder) = self.command_encoder.take() {
            self.commands
                .push(PendingCommandBuffer::Buffer(encoder.finish()));
        }
    }
}
...
impl<'w, 's> RenderContext<'w, 's> {
    ...
    pub fn add_command_buffer_generation_task(
        &mut self,
        task: impl FnOnce(CommandBufferTaskContext) -> CommandBuffer + Send + Sync + 'static,
    ) {
        self.state.0.commands.push(PendingCommandBuffer::Task {
            task: Box::new(task),
        });
    }
...
#[derive(Clone)]
pub struct CommandBufferTaskContext {
    render_device: RenderDevice,
    diagnostics_recorder: Option<DiagnosticsRecorder>,
}

impl CommandBufferTaskContext {
    /// Creates a [`CommandBufferTaskContext`]
    pub fn new(
        render_device: RenderDevice,
        diagnostics_recorder: Option<DiagnosticsRecorder>,
    ) -> Self {
    ...
    /// Returns the diagnostics recorder, if available.
    pub fn diagnostic_recorder(&self) -> Option<&DiagnosticsRecorder> {
        self.diagnostics_recorder.as_ref()
    }
...
impl<'w> FlushCommands<'w> {
    ...
    pub fn flush(&mut self, context: CommandBufferTaskContext) {
        let mut buffers = self.pending.finish(context).peekable();
