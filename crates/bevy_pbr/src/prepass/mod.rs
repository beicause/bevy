...
use bevy_render::{
    ...
    view::{
        ExtractedView, RenderVisibilityRanges, RetainedViewEntity, ViewTarget, ViewUniform,
...
pub fn check_prepass_views_need_specialization(
    ...
    for (view, view_target, depth_prepass, normal_prepass, motion_vector_prepass) in
        views.iter_mut()
    {
            ...
            for (_, visible_entity) in visible_entities.iter::<Mesh3d>() {
                ...
                match alpha_mode {
                    AlphaMode::Opaque | AlphaMode::AlphaToCoverage | AlphaMode::Mask(_) => {
                        mesh_key |= alpha_mode_pipeline_key(alpha_mode, view_target.msaa_samples());
                    }
                    AlphaMode::Blend
