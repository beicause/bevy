...
use bevy_render::{
    ...
    view::{ExtractedView, ViewDepthTexture, ViewTarget, ViewUniform, ViewUniforms},
};
...
pub(super) fn queue_render_sky_pipelines(
    views: Query<(Entity, &ViewTarget), (With<Camera>, With<ExtractedAtmosphere>)>,
    ...
    for (entity, view_target) in &views {
        let id = specializer.specialize(
            ...
            RenderSkyPipelineKey {
                msaa_samples: view_target.msaa_samples(),
...
pub(super) fn prepare_atmosphere_bind_groups(
    ...
    for (entity, atmosphere, textures, view_depth_texture, view_target) in &views {
        ...
        let render_sky = render_device.create_bind_group(
            ...
            &pipeline_cache.get_bind_group_layout(if view_target.msaa_samples() == 1 {
